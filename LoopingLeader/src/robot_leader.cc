...
void MyRobotController::init()
{
    ...
    //Set robot initial x and y positoin
    x_pos = x(); 
    y_pos = y(); 
}
...
void MyRobotController::update()
{
    ...
    //Calculate error between robot current position and robot goal
    double er = robot_pos_error(x_pos,y_pos,robot_goal.first,robot_goal.second); 
    //If error is small, index to next step (lookahead set to 3); 
    if(er < 30)
    {
